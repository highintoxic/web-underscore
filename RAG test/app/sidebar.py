...
def display_sidebar():
    ...
    if "documents" in st.session_state and st.session_state.documents:
        for doc in st.session_state.documents:
            st.sidebar.text(f"{doc['filename']} (ID: {doc['file_id']})")  # Changed from 'id' to 'file_id'

        selected_file_id = st.sidebar.selectbox("Select a document to delete", 
                                                options=[doc['file_id'] for doc in st.session_state.documents])  # Changed from 'id' to 'file_id'
